...
const std::vector<range>& range_queue::operator += ( const range_queue& r_q)
{
	...
	_range_q.swap( temp );
	ysh_assert( this->is_equal( temp_queue ) );
#endif
	return _range_q;
}
...
void range_queue::add_new_to(const range_queue &r_q, range_queue &add_q, range_queue &new_q)
{
	bool has_merged = false;
	bool src_been_trunced = false;
	_u64 merged_begin_pos, trunced_pos;
...
_u64 range_queue::all_range_length(void) const
{
	_u64 ret_val = 0;
...
bool range_queue::is_inner_range(const range& r) const
{
	...
	return ( it_find != _range_q.begin() && (it_find - 1)->end() >= r.end() 
		|| it_find != _range_q.end() && it_find->end() >= r.end() && it_find->pos() == r.pos());
}
...
void range_queue::to_string( std::string & result ) const
{
	...
	{
		sprintf( buf, "(%lu)[%I64u, %I64u, %I64u)%s", i+1, _range_q[i].pos(), _range_q[i].length(), _range_q[i].end(), 
			i<(_range_q.size()-1)?"\r\n":"" );
		result += buf;
	}
}
...
std::string range_queue::to_string( ) const
{
	...
	for ( unsigned int i=0; i<_range_q.size(); i++ )
	{
		sprintf( buf, "[%I64u,%I64u]%s", _range_q[i].pos(), _range_q[i].length(), 
			i<(_range_q.size()-1)?",":"" );
		result += buf;
	}
    return result;
...
std::string range_queue::to_string_line( ) const
{
	...
	for ( unsigned int i=0; i<_range_q.size(); i++ )
	{
		sprintf( buf, "(%lu)[%I64u, %I64u, %I64u) ", i+1, _range_q[i].pos(), _range_q[i].length(), _range_q[i].end());
		result += buf;
	}
    return result;
}
...
void range_queue::inc_offset( _u64 offset_value )
{
...
void range_queue::dec_offset( _u64 offset_value )
{
    if ( false == _range_q.empty() && offset_value )
    {
        std::vector<range>::iterator it = _range_q.begin();
